...
@app.post("/direction/")
def get_study_direction(study_direction: format_direction_for_study, existing_learning, learning_goal, client: OpenAI = Depends(get_openai_client)):
...
def get_recommended_action(recommend_action: format_recommend_action, existing_learning, learning_goal, client: OpenAI = Depends(get_openai_client)):
...
# 3에 대한 함수
@app.post("/question/")
def get_answer_question(study_direction: format_answer_for_question, user_question, client: OpenAI = Depends(get_openai_client)):
    prompt = f"Please provide an answer to the user's question about your areas of interest."
    response = client.Completion.create(
        model="gpt-3.5-turbo-1106",
        messages=[
            {
